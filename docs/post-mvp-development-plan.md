...
- SFX: удар, получение урона, подбор, смерть врага — файлы в `assets/audio/` (после `make assets-sync` → `public/assets/audio/`, URL `/assets/audio/…`).
- **Источник в репозитории:** подборка из `full-assets/RPG Sound Pack/` (см. таблицу соответствий в [`docs/agent-checklists/run-23-sound-effects.md`](./agent-checklists/run-23-sound-effects.md)); сам каталог `full-assets/` в git обычно не коммитится — в бандл попадают только копии под `assets/audio/`.
- В TWA учитывать политику автовоспроизведения: первый звук после явного жеста пользователя или кнопка «включить звук».
- Опционально фоновая музыка с низким приоритетом и настройкой громкости (в RPG Sound Pack отдельного лупа нет — добавить свой файл в `assets/audio/`, например `music-loop.ogg`).
